...
# ================= DISABLE VIBRATOR NO-OP =================
echo "Patching vibrator drivers to no-op..."
...
def replace_func(text, func_name, new_body):
    """
    Replace a C function body by matching from function name to balanced braces.
    This is safer than line-number patch.
    """
    ...
    return text[:brace] + new_body + text[end:], True


# ---------- 1. Samsung common vibrator ----------
p = Path("drivers/vibrator/common/sec_vibrator.c")
if p.exists():
    s = p.read_text(errors="ignore")

    if "GHOST_DISABLE_VIBRATOR_COMMON" not in s:
        s = s.replace(
            "#include",
            "/* GHOST_DISABLE_VIBRATOR_COMMON */\n#include",
            1
        )

    # Chặn lệnh bật rung ở tầng common.
    # Nếu en=true: bỏ qua.
    # Nếu en=false: vẫn cho gọi xuống driver để đảm bảo motor bị tắt nếu đang bật.
    s, ok1 = replace_func(
        s,
        "sec_vibrator_set_enable",
        """{
\tint ret = 0;
...
\t/* GHOST_DISABLE_VIBRATOR: block all enable requests */
\tif (en)
\t\treturn 0;

\t/* Still allow disable request to force motor off */
\tif (ddata->vib_ops && ddata->vib_ops->enable)
\t\tret = ddata->vib_ops->enable(ddata->dev, false);

\treturn ret;
}"""
    )

    # Chặn set intensity ở tầng common.
    s, ok2 = replace_func(
        s,
        "sec_vibrator_set_intensity",
        """{
\tif (!ddata)
\t\treturn -ENODATA;

\t/* GHOST_DISABLE_VIBRATOR: force intensity to zero */
\tddata->intensity = 0;
...
\treturn 0;
    ...
    # Chặn đường haptic engine bật motor.
    s, ok3 = replace_func(
        s,
        "sec_vibrator_haptic_enable",
        """{
\t/* GHOST_DISABLE_VIBRATOR: haptic enable blocked */
\treturn;
}"""
    )

    # Chặn packet haptic engine.
    s, ok4 = replace_func(
        s,
        "sec_vibrator_engine_run_packet",
        """{
\t/* GHOST_DISABLE_VIBRATOR: haptic packet blocked */
\treturn;
}"""
    )

    p.write_text(s)
    print(f"patched {p}: set_enable={ok1}, set_intensity={ok2}, haptic_enable={ok3}, packet={ok4}")
else:
    print(f"skip missing {p}")


# ---------- 2. Maxim MAX77705 motor driver ----------
p = Path("drivers/vibrator/maxim/max77705_vibrator.c")
if p.exists():
    s = p.read_text(errors="ignore")

    if "GHOST_DISABLE_VIBRATOR_MAX77705" not in s:
        s = s.replace(
            "#include",
            "/* GHOST_DISABLE_VIBRATOR_MAX77705 */\n#include",
            1
        )

    # Chặn bật motor ở tầng I2C: mọi request đều ghi OFF.
    s, ok1 = replace_func(
        s,
        "max77705_vibrator_i2c_enable",
        """{
\tint ret;

\t/* GHOST_DISABLE_VIBRATOR: always force motor OFF */
\tret = max77705_update_reg(ddata->i2c,
\t\tMAX77705_PMIC_REG_MCONFIG,
\t\t0x0, MOTOR_LRA | MOTOR_EN);

\tif (ret)
\t\tpr_err("i2c force motor off error %d\\n", ret);
...
\treturn ret;
    ...
    s, ok2 = replace_func(
        s,
        "max77705_vibrator_enable",
        """{
\tstruct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);

\t/* GHOST_DISABLE_VIBRATOR: ignore en, always force OFF */
\treturn max77705_vibrator_i2c_enable(ddata, false);
}"""
    )
    ...
    s, ok3 = replace_func(
        s,
        "max77705_vib_set_intensity",
        """{
\tstruct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);

\t/* GHOST_DISABLE_VIBRATOR: neutral PWM and disable */
\tif (ddata && ddata->pwm) {
\t\tddata->duty = ddata->period >> 1;
...
\treturn 0;
    ...
    p.write_text(s)
    print(f"patched {p}: i2c_enable={ok1}, enable={ok2}, intensity={ok3}")
else:
    print(f"skip missing {p}")
